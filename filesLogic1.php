...
$sql="SELECT * FROM sub1homework1version1";
$result = mysqli_query($conn,$sql);

$files = mysqli_fetch_all($result, MYSQLI_ASSOC);
$status = "";
// Uploads files
if (isset($_POST['save'])&& isset($_FILES['myfile1'])){ // if save button on the form is clicked
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx','doc','pptx','xlsx'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            $ins_query = "INSERT INTO sub1homework1version1 (name, size, downloads, trn_date, submittedby) 
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM sub1homework1version1 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        $updateQuery = "UPDATE sub1homework1version1 SET downloads=$newCount WHERE id=$id";
