...
$sql="SELECT * FROM sub2homework1version2";
$result = mysqli_query($conn,$sql);

$files = mysqli_fetch_all($result, MYSQLI_ASSOC);
$status = "";
// Uploads files
if (isset($_POST['save'])&& isset($_FILES['myfile5'])) { // if save button on the form is clicked
    // name of the uploaded file
	session_start();
    $filename = $_FILES['myfile5']['name'];
    ...
    } elseif ($_FILES['myfile5']['size'] > 1000000) { // file shouldn't be larger than 1Megabyte
        Print'<script>alert("File too large!");</script>';
		Print'<script>window.location.assign("stu1subject2.php");</script>';
    } else {
        // move the uploaded (temporary) file to the specified destination
        if (move_uploaded_file($file, $destination)) {
            $ins_query = "INSERT INTO sub2homework1version2 (name, size, downloads, trn_date, submittedby) 
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM sub2homework1version2 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        $updateQuery = "UPDATE sub2homework1version2 SET downloads=$newCount WHERE id=$id";
