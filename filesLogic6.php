...
$sql="SELECT * FROM sub2homework1version3";
$result = mysqli_query($conn,$sql);

$files = mysqli_fetch_all($result, MYSQLI_ASSOC);
$status = "";
// Uploads files
if (isset($_POST['save'])&& isset($_FILES['myfile6'])) { // if save button on the form is clicked
    // name of the uploaded file
	session_start();
    $filename = $_FILES['myfile6']['name'];
    ...
    $file = $_FILES['myfile6']['tmp_name'];
    $size = $_FILES['myfile6']['size'];
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx','doc','pptx','xlsx'])) {
		...
		Print'<script>window.location.assign("stu1subject2.php");</script>';
   
    } elseif ($_FILES['myfile6']['size'] > 1000000) { // file shouldn't be larger than 1Megabyte
        Print'<script>alert("File too large!");</script>';
		Print'<script>window.location.assign("stu1subject2.php");</script>';
    } else {
        // move the uploaded (temporary) file to the specified destination
        if (move_uploaded_file($file, $destination)) {
            $ins_query = "INSERT INTO sub2homework1version3 (name, size, downloads, trn_date, submittedby) 
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM sub2homework1version3 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
    ...
    if (file_exists($filepath)) {
        ...
        $updateQuery = "UPDATE sub2homework1version3 SET downloads=$newCount WHERE id=$id";
